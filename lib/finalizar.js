...
import { chaveSemCabeca } from './cabecas.js';
import { aplicarRostoReal } from './rostoreal.js';
 ...
 */
export async function finalizarFoto({ selfieId, predictionId, outputUrl, inputImageUrl, selfieUrl }) {
  ...
  // Templates "sem-cabeca-*" passam pelo modelo de edicao por prompt, que
  // reenquadra a imagem: nao da para colar nada de volta por coordenada, e o
  // politico ja vem preservado pelo proprio prompt. So os "tapado-*" (face swap,
  // que devolve a imagem no mesmo enquadramento) precisam da restauracao.
  const semCabeca = String(inputImageUrl || '').includes('sem-cabeca-');
  const chave = semCabeca ? null : chaveDoTapado(inputImageUrl);
  const config = chave ? CAIXAS[chave] : null;

  let foto = await baixar(outputUrl);

  // Passada final: o Gemini desenhou a cabeca com o cabelo certo, mas o rosto e
  // uma recriacao. O face swap por cima devolve os pixels REAIS do cliente.
  if (semCabeca && selfieUrl) {
    const chaveCabeca = chaveSemCabeca(inputImageUrl);
    if (chaveCabeca) {
      foto = await aplicarRostoReal({ imagem: foto, chave: chaveCabeca, selfieUrl, selfieId, predictionId });
    }
  }
  ...
  if (config) {
    ...
    if (!semCabeca) console.warn(`[Finalizar] template nao reconhecido em ${inputImageUrl} - salvando sem restaurar`);
  }
